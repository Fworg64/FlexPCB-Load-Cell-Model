...
from optimizations.nag import NesterovAcceleratedGradient
from optimizations.optimization_solver_base import OptimizationSolver


def test_obj(x):
    A = np.asarray([[2, 1], [0, 1]], dtype=float)
    value = x.T @ A @ x
    return value
...
class TestGradientDescent(unittest.TestCase):
    def test_isOptimizationSolver(self):
        self.assertTrue(issubclass(NesterovAcceleratedGradient, OptimizationSolver))

    def test_can_instantiate(self):
        my_gd_optimizer = NesterovAcceleratedGradient(test_obj, test_obj_grad, np.asarray([[1], [1]]), 1e-4)
    ...
    def test_can_minimize_obj_tol(self):
        x0 = np.asarray([1, 1])
        my_gd_optimizer = NesterovAcceleratedGradient(test_obj, test_obj_grad, x0, 1e-4)
        my_gd_optimizer.set_params(0.227, 0.25)
